...
namespace Monke.Shared
{
    public static class StringExtensions
    {
        private static Dictionary<ushort, string> hashToText = new Dictionary<ushort, string>();
        public static ushort ToUint16Hash(this string value)
        {
            var hash = Crc16.ComputeHash(value);
            if (!hashToText.ContainsKey(hash))
            {
                hashToText[hash] = value;
            }
            return hash;
        }

        public static string FromHash(ushort value)
        {
            if (hashToText.ContainsKey(value))
            {
